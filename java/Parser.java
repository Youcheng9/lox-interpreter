...
//> unary
  private Expr unary() {
    ...
    // Error productions for binary operators that appear without a
    // left-hand operand. Report the error, then consume the right-hand
    // operand at the operator's normal precedence to avoid cascading errors.
    if (match(OR)) {
    ...
    // MINUS is excluded because it is also a valid unary operator.
    if (match(PLUS)) {
      error(previous(), "Missing left-hand operand.");
      factor();
      return null;
